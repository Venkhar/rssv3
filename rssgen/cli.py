...
async def fetch(client: httpx.AsyncClient, url: str, cache: SimpleCache, *, timeout=20) -> bytes:
...
async def process_one(
    ...
    force_relative = bool(src.get("force_relative", False))
    site_name = src.get("site_name")
    feed_title = src.get("feed_title")
    out_name = src.get("out") or "feed.xml"

    html = await fetch(client, url, cache)
    items = extract_items(
    ...
    if fetch_detail:
        ...
        for it in items[:max_items]:
            try:
                detail = await fetch(client, it.url, cache)
                it = enrich_from_detail(detail.decode("utf-8", errors="ignore"), it)
                await asyncio.sleep(0.02)  # politesse
            except Exception:
                pass
            enriched.append(it)
        items = enriched
    ...
    with open(path, "wb") as f:
        f.write(feed_bytes)
    print(f"✅ {url} → {path} ({len(items)} entrées)")
    return path, len(items)


async def run_batch(
    sources: list[dict], *, concurrency: int, cache_ttl: int, out_dir: str, max_items: int
) -> int:
    cache = SimpleCache(ttl_seconds=cache_ttl)
    limits = httpx.Limits(max_connections=concurrency, max_keepalive_connections=concurrency)
    async with httpx.AsyncClient(
        headers=HEADERS, limits=limits, http2=True, follow_redirects=True
    ) as client:
        sem = asyncio.Semaphore(concurrency)

        async def worker(src: dict):
            async with sem:
                await process_one(client, src, cache, out_dir, max_items)
...
async def run_single(url: str, **kwargs) -> int:
    ...
    return await run_batch(
        ...
        cache_ttl=kwargs.get("cache_ttl", 1800),
        out_dir=kwargs.get("out_dir", "dist"),
        max_items=kwargs.get("max_items", 25),
    )
